...
void memoryCleanup(Node* list,Node* list2,Node* r_list, FILE* f_list, FILE* f_list2){
...
int main(int argc, char **argv){
    ...
    FILE* f_list = NULL; FILE* f_list2 =  NULL;
    ...
    if(argc <= 1){
        ...
        for(int i = 0; i < argc; i++){
                    ...
                    //runAll(head);
                        ...
                        switch (testNum){
                            case 1: {
                                ...
                                test1(r_list, f_list);
                                memoryCleanup(list, list2, r_list, f_list, f_list2);
                                break;
                            }
                            case 2: {
            ...
            if(!strcmp(argv[i], "-p")){
                    if(!printList(list)) {printf("LIST 1: \n"); printf("\n");}
                    if(!printList(list2)) {printf("LIST 2: \n"); printf("\n");}
                    printf("RESULT LIST: \n");
                    printList(r_list);
            }
        }
